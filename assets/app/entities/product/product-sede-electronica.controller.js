...
(function () {
        'use strict';
        angular.module("wpc")
            .controller('ProductSedeElectronicaController', ProductSedeElectronicaController);

        ProductSedeElectronicaController.$inject =
            ['$scope', 'GarantiasServices', 'GarantiasService', 'GarantiasServiceGetGarantias',
                'GarantiasServiceUpdateGarantias', '$location', 'ngTableParams', '$filter'];

        function ProductSedeElectronicaController($scope, GarantiasServices, GarantiasService, GarantiasServiceGetGarantias,
                                                  GarantiasServiceUpdateGarantias, $location, ngTableParams, $filter) {
            $scope.all_columns = [];
            $scope.columns = [];
            $scope.digital = [];
            ...
            $scope.loadPlanillaToTula = function () {
                ...
                $scope.columns = [];
                $scope.all_columns = [];
                $scope.digital = GarantiasServiceGetGarantias.show({
        ...
        function datosFillComplementarios($scope) {
            ...
            if ($scope.rowDetail.tipoGarantia == 6210 || true) {//hipotecaria
                $scope.datosCoplementarios[0] = {
                    key: "pagareOriginalPrendiaria",
                    value: $scope.rowDetail["pagareOriginalPrendiaria"] ? $scope.rowDetail["pagareOriginalPrendiaria"] : false
                };
                $scope.datosCoplementarios[1] = {
                    key: "cartaInstrucciones",
                    value: $scope.rowDetail["cartaInstrucciones"] ? $scope.rowDetail["cartaInstrucciones"] : false
                };
                $scope.datosCoplementarios[2] = {
                    key: "copiaMeritoEjecutivo",
                    value: $scope.rowDetail["copiaMeritoEjecutivo"] ? $scope.rowDetail["copiaMeritoEjecutivo"] : false
                };
                $scope.datosCoplementarios[3] = {
                    key: "folioMatriculaInmobiliaria",
                    value: $scope.rowDetail["folioMatriculaInmobiliaria"] ? $scope.rowDetail["folioMatriculaInmobiliaria"] : false
                };
                $scope.datosCoplementarios[4] = {
                    key: "tieneHuella",
                    value: $scope.rowDetail["tieneHuella"] ? $scope.rowDetail["tieneHuella"] : false
                };
